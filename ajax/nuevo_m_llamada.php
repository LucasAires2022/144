...
if (!empty($_POST['descr']) && !empty($_POST['activo'])) {
    ...
    $sql = "SELECT * FROM motivo_llamada WHERE descr = '" . $alta_descr . "';";
    ...
    if ($query_check_dato == 1) {
        ...
        $sql = "SELECT * FROM motivo_llamada WHERE orden = (SELECT MAX( orden )  FROM motivo_llamada);";
        ...
        if ($query_check_orden_num != 1) {
            ...
            $sql = "INSERT INTO motivo_llamada (descr, activo, orden)

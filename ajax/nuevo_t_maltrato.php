...
if (!empty($_POST['descr']) && !empty($_POST['activo'])) {
    ...
    $sql = "SELECT * FROM tipo_maltrato WHERE descr = '" . $alta_descr . "';";
    ...
    if ($query_check_dato == 1) {
        ...
        if ($query_check_vacio_num == 0) {
            ...
            $sql = "SELECT * FROM tipo_maltrato WHERE orden = (SELECT MAX( orden )  FROM tipo_maltrato);";

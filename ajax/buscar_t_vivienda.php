...
if ($action == 'ajax') {
    ...
    $sqlorden = "SELECT * FROM tenencia_vivienda WHERE orden = (SELECT MAX( orden )  FROM tenencia_vivienda);";
    /*
      SELECT *
    ...
    if (isset($ordenar) && $ordenar != "") {
        /* con_log("pasa2"); */
        $_SESSION['orden_abm_t_vivienda'] = $ordenar;
    ...
    $count_query = mysqli_query($con, "SELECT count(*) AS numrows FROM tenencia_vivienda $sWhere");
    $row = mysqli_fetch_array($count_query);
    $numrows = $row['numrows'];
    $total_pages = ceil($numrows / $per_page);
    $reload = '../abm/tenencia_vivienda.php';
    //main query to fetch the data
    $sql = "SELECT * FROM tenencia_vivienda $sWhere LIMIT $offset,$per_page";

...
if ($action == 'ajax') {
    ...
    $sqlorden = "SELECT * FROM tipo_maltrato WHERE orden = (SELECT MAX( orden )  FROM tipo_maltrato);";
    /*
      SELECT *
    ...
    if (isset($ordenar) && $ordenar != "") {
        /* con_log("pasa2"); */
        $_SESSION['orden_abm_t_maltrato'] = $ordenar;
    ...
    $count_query = mysqli_query($con, "SELECT count(*) AS numrows FROM tipo_maltrato $sWhere");
    $row = mysqli_fetch_array($count_query);
    $numrows = $row['numrows'];
    $total_pages = ceil($numrows / $per_page);
    $reload = '../abm/tipo_maltrato.php';
    //main query to fetch the data
    $sql = "SELECT * FROM tipo_maltrato $sWhere LIMIT $offset,$per_page";

...
if (!empty($_GET['id']) && !empty($_GET['pos']) && !empty($_GET['dir'])) {
    ...
    if ($dato_target >= 1) {
        ...
        $sql = "SELECT * FROM medidas_proteccion WHERE orden = '" . $dato_target . "' ;";
        ...
        if ($query_check_dato == 1) {
            ...
            $sql = "UPDATE medidas_proteccion SET orden = IF(id=$dato_id,'$dato_target','$dato_orden') WHERE id IN ('$dato_id','$prev_id')";
            //con_log($sql);
            ...
            $sql = "UPDATE medidas_proteccion SET orden='" . $dato_target . "' WHERE id='" . $dato_id . "';";
            $query_update = mysqli_query($con, $sql);

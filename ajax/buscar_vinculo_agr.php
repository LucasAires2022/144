...
if ($action == 'ajax') {
    ...
    $sqlorden = "SELECT * FROM vinculo_agresor WHERE orden = (SELECT MAX( orden )  FROM vinculo_agresor);";
    /*
      SELECT *
    ...
    if (isset($ordenar) && $ordenar != "") {
        /* con_log("pasa2"); */
        $_SESSION['orden_abm_vinculo_agr'] = $ordenar;
    ...
    $count_query = mysqli_query($con, "SELECT count(*) AS numrows FROM vinculo_agresor $sWhere");
    $row = mysqli_fetch_array($count_query);
    $numrows = $row['numrows'];
    $total_pages = ceil($numrows / $per_page);
    $reload = '../abm/vinculo_agresor.php';
    //main query to fetch the data
    $sql = "SELECT * FROM vinculo_agresor $sWhere LIMIT $offset,$per_page";

...
if ($action == 'ajax') {
    ...
    $sqlorden = "SELECT * FROM origen_llamado WHERE orden = (SELECT MAX( orden )  FROM origen_llamado);";
    /*
      SELECT *
    ...
    if (isset($ordenar) && $ordenar != "") {
        /* con_log("pasa2"); */
        $_SESSION['orden_abm_o_llamado'] = $ordenar;
    ...
    $count_query = mysqli_query($con, "SELECT count(*) AS numrows FROM origen_llamado $sWhere");
    $row = mysqli_fetch_array($count_query);
    $numrows = $row['numrows'];
    $total_pages = ceil($numrows / $per_page);
    $reload = '../abm/origen_llamado.php';
    //main query to fetch the data
    $sql = "SELECT * FROM origen_llamado $sWhere LIMIT $offset,$per_page";

...
if ($action == 'ajax') {
    ...
    $sqlorden = "SELECT * FROM derivacion WHERE orden = (SELECT MAX( orden )  FROM derivacion);";
    /*
      SELECT *
    ...
    if (isset($ordenar) && $ordenar != "") {
        /* con_log("pasa2"); */
        $_SESSION['orden_abm_derivacion'] = $ordenar;
    ...
    if (/* $_GET['q'] */$q != "") {
        ...
        foreach ($criterio as $cri) {
            $cri = trim($cri);
            if ($contador == 0) {
                $sWhere = " WHERE (d.descr like '%$cri%') ";
            } else {
                $sWhere .= " AND (d.descr like '%$cri%') ";
            }
            $contador++;
            /* con_log("Cri es:$cri"); */
        }
    ...
    con_log("q_motivo es: $q_motivo - ");
    if ($q_motivo <> "0") {
        if ($sWhere == "") {
            $sWhere = " WHERE (d.motivo_consulta_id = '$q_motivo') ";
        } else {
            $sWhere .= " AND (d.motivo_consulta_id = '$q_motivo') ";
        }
    }
    ...
    $sqljoin = "SELECT DISTINCT d.descr as descr, d.id as id, d.activo as activo, d.orden as orden, m.descr as motivo, m.id as idmot "
            . "FROM derivacion d "
            . "LEFT JOIN motivo_consulta m ON m.id = d.motivo_consulta_id ";
    $sqljoincount = "SELECT DISTINCT count(*) AS numrows, d.descr as descr, d.id as id, d.activo as activo, d.orden as orden, m.descr as motivo, m.id as idmot "
            . "FROM derivacion d "            
            . "LEFT JOIN motivo_consulta m ON m.id = d.motivo_consulta_id ";
            ...
            <table class="table table-hover <? /* table-striped table-bordered table-hover */ ?>" style="margin-bottom:0px;">
                <tr  class="abm-header">
                    ...
                    <th class="">
                        ...
                        <? } else { ?>
                            <a class="abm-header-col" href='javascript:load_orden(1,"aorden");'>
                                Orden <i class="fas fa-caret-down" style='color:transparent;'></i></a>
                        <? } ?>
                    </th>
                    <th class="">
                        ...
                        <? } else { ?>
                            <a class="abm-header-col" href='javascript:load_orden(1,"adescr");'>
                                Vínculo <i class="fas fa-caret-down" style='color:transparent;'></i></a>
                        <? } ?>
                    </th>
                    <th class="">
                        <? if ($ordenar == "motivo" && $ascdesc == "asc") { ?>
                            <a class="abm-header-col" href='javascript:load_orden(1,"dmotivo");'>
                                Motivo de consulta <i class="fas fa-caret-up"></i></a>
                        <? } else if ($ordenar == "motivo" && $ascdesc == "desc") { ?>
                            <a class="abm-header-col" href='javascript:load_orden(1,"amotivo");'>
                                Motivo de consulta <i class="fas fa-caret-down"></i></a>
                        <? } else { ?>
                            <a class="abm-header-col" href='javascript:load_orden(1,"amotivo");'>
                                Motivo de consulta <i class="fas fa-caret-down" style='color:transparent;'></i></a>
                        <? } ?>
                    </th>
                ...
                while ($row = mysqli_fetch_array($query)) {
                    ...
                    <input type="hidden" value="<?php echo $bumotivo; ?>" id="emotivo<?php echo $id_dato; ?>">

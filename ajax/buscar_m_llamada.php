...
if ($action == 'ajax') {
    ...
    $sqlorden = "SELECT * FROM motivo_llamada WHERE orden = (SELECT MAX( orden )  FROM motivo_llamada);";
    /*
      SELECT *
    ...
    if (isset($ordenar) && $ordenar != "") {
        /* con_log("pasa2"); */
        $_SESSION['orden_abm_m_llamada'] = $ordenar;
    ...
    $count_query = mysqli_query($con, "SELECT count(*) AS numrows FROM motivo_llamada $sWhere");
    $row = mysqli_fetch_array($count_query);
    $numrows = $row['numrows'];
    $total_pages = ceil($numrows / $per_page);
    $reload = '../abm/motivo_llamada.php';
    //main query to fetch the data
    $sql = "SELECT * FROM motivo_llamada $sWhere LIMIT $offset,$per_page";

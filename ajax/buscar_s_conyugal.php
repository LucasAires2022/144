...
if ($action == 'ajax') {
    ...
    $sqlorden = "SELECT * FROM situacion_conyugal WHERE orden = (SELECT MAX( orden )  FROM situacion_conyugal);";
    /*
      SELECT *
    ...
    if (isset($ordenar) && $ordenar != "") {
        /* con_log("pasa2"); */
        $_SESSION['orden_abm_s_conyugal'] = $ordenar;
    ...
    $count_query = mysqli_query($con, "SELECT count(*) AS numrows FROM situacion_conyugal $sWhere");
    $row = mysqli_fetch_array($count_query);
    $numrows = $row['numrows'];
    $total_pages = ceil($numrows / $per_page);
    $reload = '../abm/situacion_conyugal.php';
    //main query to fetch the data
    $sql = "SELECT * FROM situacion_conyugal $sWhere LIMIT $offset,$per_page";

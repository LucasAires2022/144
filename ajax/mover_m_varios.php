...
if (!empty($_GET['id']) && !empty($_GET['pos']) && !empty($_GET['dir'])) {
    ...
    if ($dato_target >= 1) {
        ...
        $sql = "SELECT * FROM motivo_varios WHERE orden = '" . $dato_target . "' ;";
        ...
        if ($query_check_dato == 1) {
            ...
            $sql = "UPDATE motivo_varios SET orden = IF(id=$dato_id,'$dato_target','$dato_orden') WHERE id IN ('$dato_id','$prev_id')";
            //con_log($sql);
            ...
            $sql = "UPDATE motivo_varios SET orden='" . $dato_target . "' WHERE id='" . $dato_id . "';";
            $query_update = mysqli_query($con, $sql);

...
if ($action == 'ajax') {
    ...
    $sqlorden = "SELECT * FROM motivo_problematica WHERE orden = (SELECT MAX( orden )  FROM motivo_problematica);";
    /*
      SELECT *
    ...
    if (isset($ordenar) && $ordenar != "") {
        /* con_log("pasa2"); */
        $_SESSION['orden_abm_m_problematica'] = $ordenar;
    ...
    $count_query = mysqli_query($con, "SELECT count(*) AS numrows FROM motivo_problematica $sWhere");
    $row = mysqli_fetch_array($count_query);
    $numrows = $row['numrows'];
    $total_pages = ceil($numrows / $per_page);
    $reload = '../abm/motivo_problematica.php';
    //main query to fetch the data
    $sql = "SELECT * FROM motivo_problematica $sWhere LIMIT $offset,$per_page";

...
if (!empty($_POST['descr']) && !empty($_POST['activo'])) {
    ...
    $sql = "SELECT * FROM modalidad_violencia WHERE descr = '" . $alta_descr . "';";
    ...
    if ($query_check_dato == 1) {
        ...
        if ($query_check_vacio_num == 0) {
            ...
            $sql = "SELECT * FROM modalidad_violencia WHERE orden = (SELECT MAX( orden )  FROM modalidad_violencia);";

...
if ($action == 'ajax') {
    ...
    $sqlorden = "SELECT * FROM medidas_proteccion WHERE orden = (SELECT MAX( orden )  FROM medidas_proteccion);";
    /*
      SELECT *
    ...
    if (isset($ordenar) && $ordenar != "") {
        /* con_log("pasa2"); */
        $_SESSION['orden_abm_m_proteccion'] = $ordenar;
    ...
    $count_query = mysqli_query($con, "SELECT count(*) AS numrows FROM medidas_proteccion $sWhere");
    $row = mysqli_fetch_array($count_query);
    $numrows = $row['numrows'];
    $total_pages = ceil($numrows / $per_page);
    $reload = '../abm/medidas_proteccion.php';
    //main query to fetch the data
    $sql = "SELECT * FROM medidas_proteccion $sWhere LIMIT $offset,$per_page";

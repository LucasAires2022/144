...
if ($action == 'ajax') {
    ...
    $sqlorden = "SELECT * FROM actividad WHERE orden = (SELECT MAX( orden )  FROM actividad);";
    /*
      SELECT *
    ...
    if (isset($ordenar) && $ordenar != "") {
        /* con_log("pasa2"); */
        $_SESSION['orden_abm_actividad'] = $ordenar;
    ...
    $count_query = mysqli_query($con, "SELECT count(*) AS numrows FROM actividad $sWhere");
    $row = mysqli_fetch_array($count_query);
    $numrows = $row['numrows'];
    $total_pages = ceil($numrows / $per_page);
    $reload = '../abm/actividad.php';
    //main query to fetch the data
    $sql = "SELECT * FROM actividad $sWhere LIMIT $offset,$per_page";

...
if (!empty($_POST['descr']) && !empty($_POST['activo'])) {
    ...
    $sql = "SELECT * FROM actividad WHERE descr = '" . $alta_descr . "';";
    ...
    if ($query_check_dato == 1) {
        ...
        if ($query_check_vacio_num == 0) {
            ...
            $sql = "INSERT INTO actividad (descr, activo, orden)
            ...
            $sql = "SELECT * FROM actividad WHERE orden = (SELECT MAX( orden )  FROM actividad);";

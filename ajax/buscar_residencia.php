...
if ($action == 'ajax') {
    ...
    $sqlorden = "SELECT * FROM lugar_residencia WHERE orden = (SELECT MAX( orden )  FROM lugar_residencia);";
    /*
      SELECT *
    ...
    if (isset($ordenar) && $ordenar != "") {
        /*con_log("pasa2");*/
        $_SESSION['orden_abm_residencia'] = $ordenar;
    ...
    $count_query = mysqli_query($con, "SELECT count(*) AS numrows FROM lugar_residencia $sWhere");
    $row = mysqli_fetch_array($count_query);
    $numrows = $row['numrows'];
    $total_pages = ceil($numrows / $per_page);
    $reload = '../abm/residencia.php';
    //main query to fetch the data
    $sql = "SELECT * FROM lugar_residencia $sWhere LIMIT $offset,$per_page";
        ...
        <div class="table-responsive">
            <table class="table table-hover <? /* table-striped table-bordered table-hover */ ?>" style="margin-bottom:0px;">
                ...
                while ($row = mysqli_fetch_array($query)) {
                    $id_residencia = $row['id'];
                    $budescr = $row['descr'];
                    $buactivo = $row['activo'];
                    $buorden = $row['orden'];
                    ?>
                    <input type="hidden" value="<?php echo $budescr; ?>" id="edescr<?php echo $id_residencia; ?>">
                    <input type="hidden" value="<?php echo $buactivo; ?>" id="eactivo<?php echo $id_residencia; ?>">
                    <input type="hidden" value="<?php echo $buorden; ?>" id="eorden<?php echo $id_residencia; ?>">
                    ...
                    <tr>
                        ...
                        ?>>
                                <? /* if ($buorden > $min_orden) { ?>
                                  <a href="javascript:mover('<?php echo $id_residencia; ?>', '<? echo $buorden; ?>','u' )" class='abm-header-col' title='Subir' ><i class="fas fa-chevron-up"></i></a>
                                  <? } else { ?>
                                  <span class='abm-disabled'><i class="fas fa-chevron-up"></i></span>
                                  <? } */ ?>
                                <?php echo $buorden; ?>
                                <? /* if ($max_orden == 0 || $buorden < $max_orden) { ?>
                                  <a href="javascript:mover('<?php echo $id_residencia; ?>', '<? echo $buorden; ?>','d' )" class='abm-header-col' title='Bajar' ><i class="fas fa-chevron-down"></i></a>
                                  <? } else { ?>
                        ...
                        <td class="tabla-acciones text-center">
                            <? if ($buorden > $min_orden) { ?>
                                <a href="javascript:mover('<?php echo $id_residencia; ?>', '<? echo $buorden; ?>','u', '<? echo $_GET['orden'];?>' )" class='abm-header-col' title='Subir' ><i class="fas fa-chevron-up" <? /* style="font-size:1.2rem;" */ ?>></i></a> 
                                <?
                                if ($max_orden == 0 || $buorden < $max_orden) {
                                    ?>&nbsp;&nbsp;<?
                                    }
                            ...
                            <? if ($max_orden == 0 || $buorden < $max_orden) { ?>
                                <a href="javascript:mover('<?php echo $id_residencia; ?>', '<? echo $buorden; ?>','d', '<? echo $_GET['orden'];?>' )" class='abm-header-col' title='Bajar' ><i class="fas fa-chevron-down" <? /* style="font-size:1.2rem;" */ ?>></i></a> 
                            <? } /* else { ?>
                              <span class='abm-disabled'><i class="fas fa-chevron-down"></i></span>
                              <? } */ ?>
                        ...
                        <td class="tabla-acciones text-center">
                            <button onclick="obtener_datos('<?php echo $id_residencia; ?>');" class='btn btn-light btn-buscar btn-acciones' data-toggle="modal" data-target="#myModal2" title='Modificar lugar de residencia' ><i class="fas fa-edit"></i></button> 
                            <? if ($buactivo == "s") { ?>
                            <button onclick="obtener_datos_toggle('<?php echo $id_residencia; ?>', 'n');" class='btn btn-light btn-buscar btn-acciones' data-toggle="modal" data-target="#toggle_activo" title='Deshabilitar lugar de residencia' ><i class="fas fa-ban"></i></button> 
                            ...
                            <? } else { ?>
                                <button onclick="obtener_datos_toggle('<?php echo $id_residencia; ?>', 's');" class='btn btn-light btn-buscar btn-acciones' data-toggle="modal" data-target="#toggle_activo" title='Habilitar lugar de residencia' ><i class="far fa-check-circle"></i></button> 

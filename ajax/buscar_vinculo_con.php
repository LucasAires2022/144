...
if ($action == 'ajax') {
    ...
    $sqlorden = "SELECT * FROM vinculo_consultante WHERE orden = (SELECT MAX( orden )  FROM vinculo_consultante);";
    /*
      SELECT *
    ...
    if (isset($ordenar) && $ordenar != "") {
        /* con_log("pasa2"); */
        $_SESSION['orden_abm_vinculo_con'] = $ordenar;
    ...
    if (/* $_GET['q'] */$q != "") {
        ...
        foreach ($criterio as $cri) {
            $cri = trim($cri);
            if ($contador == 0) {
                $sWhere = " WHERE (vinculo like '%$cri%') ";
            } else {
                $sWhere .= " AND (vinculo like '%$cri%') ";
            }
            $contador++;
            /* con_log("Cri es:$cri"); */
        }
    ...
    $count_query = mysqli_query($con, "SELECT count(*) AS numrows FROM vinculo_consultante $sWhere");
    $row = mysqli_fetch_array($count_query);
    $numrows = $row['numrows'];
    $total_pages = ceil($numrows / $per_page);
    $reload = '../abm/vinculo_consultante.php';
    //main query to fetch the data
    $sql = "SELECT * FROM vinculo_consultante $sWhere LIMIT $offset,$per_page";

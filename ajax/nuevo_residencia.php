...
if (!empty($_POST['descr']) && !empty($_POST['activo'])) {
    ...
    $sql = "SELECT * FROM lugar_residencia WHERE descr = '" . $alta_descr . "';";
    ...
    if ($query_check_residencia == 1) {
        ...
        if ($query_check_vacio_num == 0) {
            ...
            $sql = "SELECT * FROM lugar_residencia WHERE orden = (SELECT MAX( orden )  FROM lugar_residencia);";
            /*
              SELECT *
            ...
            if ($query_check_orden_num != 1) {
                ...
                $sql = "INSERT INTO lugar_residencia (descr, activo, orden)
                            VALUES('" . $alta_descr . "','" . $alta_activo . "','" . $alta_orden . "');";
                $query_new_residencia_insert = mysqli_query($con, $sql);


                if ($query_new_residencia_insert) {
                    echo 1; //"El usuario ha sido creado con éxito.";
                } else {
                    echo 4; //"No se pudo guardar el lugar de residencia. Por favor, vuelva a intentarlo.";
                }

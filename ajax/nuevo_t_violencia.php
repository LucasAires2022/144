...
if (!empty($_POST['descr']) && !empty($_POST['activo'])) {
    ...
    $sql = "SELECT * FROM tipo_violencia WHERE descr = '" . $alta_descr . "';";
    ...
    if ($query_check_dato == 1) {
        ...
        if ($query_check_vacio_num == 0) {
            ...
            $sql = "INSERT INTO tipo_violencia (descr, activo, orden)
            ...
            $sql = "SELECT * FROM tipo_violencia WHERE orden = (SELECT MAX( orden )  FROM tipo_violencia);";

...
if ($action == 'ajax') {
    ...
    $sqlorden = "SELECT * FROM tipo_violencia WHERE orden = (SELECT MAX( orden )  FROM tipo_violencia);";
    /*
      SELECT *
    ...
    if (isset($ordenar) && $ordenar != "") {
        /* con_log("pasa2"); */
        $_SESSION['orden_abm_t_violencia'] = $ordenar;
    ...
    $count_query = mysqli_query($con, "SELECT count(*) AS numrows FROM tipo_violencia $sWhere");
    $row = mysqli_fetch_array($count_query);
    $numrows = $row['numrows'];
    $total_pages = ceil($numrows / $per_page);
    $reload = '../abm/tipo_violencia.php';
    //main query to fetch the data
    $sql = "SELECT * FROM tipo_violencia $sWhere LIMIT $offset,$per_page";

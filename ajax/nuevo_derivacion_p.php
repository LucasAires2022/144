...
if (!empty($_POST['descr']) && !empty($_POST['activo'])) {
    ...
    $sql = "SELECT * FROM derivacion WHERE motivo_consulta_id = 2 AND descr = '" . $alta_descr . "';";
    ...
    if ($query_check_dato == 1) {
        ...
        $sql = "SELECT * FROM derivacion WHERE motivo_consulta_id = 2;";
        $query_check_vacio = mysqli_query($con, $sql);
        $query_check_vacio_num = mysqli_num_rows($query_check_vacio);
        if ($query_check_vacio_num == 0) {
            $alta_orden = 1;

            $sql = "INSERT INTO derivacion (descr, activo, orden, motivo_consulta_id)
                            VALUES('" . $alta_descr . "','" . $alta_activo . "','" . $alta_orden . "' , 2);";
            ...
            $sql = "SELECT * FROM derivacion WHERE motivo_consulta_id = 2 AND orden = (SELECT MAX( orden )  FROM derivacion WHERE motivo_consulta_id = 2);";

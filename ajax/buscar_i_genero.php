...
if ($action == 'ajax') {
    ...
    $sqlorden = "SELECT * FROM identidad_genero WHERE orden = (SELECT MAX( orden )  FROM identidad_genero);";
    /*
      SELECT *
    ...
    if (isset($ordenar) && $ordenar != "") {
        /* con_log("pasa2"); */
        $_SESSION['orden_abm_i_genero'] = $ordenar;
    ...
    $count_query = mysqli_query($con, "SELECT count(*) AS numrows FROM identidad_genero $sWhere");
    $row = mysqli_fetch_array($count_query);
    $numrows = $row['numrows'];
    $total_pages = ceil($numrows / $per_page);
    $reload = '../abm/identidad_genero.php';
    //main query to fetch the data
    $sql = "SELECT * FROM identidad_genero $sWhere LIMIT $offset,$per_page";

...
if ($action == 'ajax') {
    ...
    $sqlorden = "SELECT * FROM motivo_varios WHERE orden = (SELECT MAX( orden )  FROM motivo_varios);";
    /*
      SELECT *
    ...
    if (isset($ordenar) && $ordenar != "") {
        /* con_log("pasa2"); */
        $_SESSION['orden_abm_m_varios'] = $ordenar;
    ...
    $count_query = mysqli_query($con, "SELECT count(*) AS numrows FROM motivo_varios $sWhere");
    $row = mysqli_fetch_array($count_query);
    $numrows = $row['numrows'];
    $total_pages = ceil($numrows / $per_page);
    $reload = '../abm/motivo_varios.php';
    //main query to fetch the data
    $sql = "SELECT * FROM motivo_varios $sWhere LIMIT $offset,$per_page";

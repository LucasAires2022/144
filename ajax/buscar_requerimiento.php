...
if ($action == 'ajax') {
    ...
    $sqlorden = "SELECT * FROM requerimiento WHERE orden = (SELECT MAX( orden )  FROM requerimiento);";
    /*
      SELECT *
    ...
    if (isset($ordenar) && $ordenar != "") {
        /* con_log("pasa2"); */
        $_SESSION['orden_abm_requerimiento'] = $ordenar;
    ...
    $count_query = mysqli_query($con, "SELECT count(*) AS numrows FROM requerimiento $sWhere");
    $row = mysqli_fetch_array($count_query);
    $numrows = $row['numrows'];
    $total_pages = ceil($numrows / $per_page);
    $reload = '../abm/requerimiento.php';
    //main query to fetch the data
    $sql = "SELECT * FROM requerimiento $sWhere LIMIT $offset,$per_page";

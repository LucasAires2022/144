...
if ($action == 'ajax') {
    ...
    $sqlorden = "SELECT * FROM nacionalidad WHERE orden = (SELECT MAX( orden )  FROM nacionalidad);";
    /*
      SELECT *
    ...
    if (isset($ordenar) && $ordenar != "") {
        /* con_log("pasa2"); */
        $_SESSION['orden_abm_nacionalidad'] = $ordenar;
    ...
    $count_query = mysqli_query($con, "SELECT count(*) AS numrows FROM nacionalidad $sWhere");
    $row = mysqli_fetch_array($count_query);
    $numrows = $row['numrows'];
    $total_pages = ceil($numrows / $per_page);
    $reload = '../abm/nacionalidad.php';
    //main query to fetch the data
    $sql = "SELECT * FROM nacionalidad $sWhere LIMIT $offset,$per_page";

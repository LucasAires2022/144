...
$('#selectMotivo').change(function () {
    var selectedItem = $('#selectMotivo').val();
    $('.formulario').fadeOut(500);
    //alert(selectedItem);
    if (selectedItem === "B") {
        $('#formB').fadeIn(500);
        $('.botones-form').fadeIn(500);
...
function mostrar_emergencia() {

    if (!$('#toggleEmergenciaSi').attr('disabled')) {

        $("#selectMotivo").attr('disabled', true);
        $("#selectMotivo").selectpicker('val', 0);
...
function ocultar_emergencia() {
    if (!$('#toggleEmergenciaNo').attr('disabled')) {

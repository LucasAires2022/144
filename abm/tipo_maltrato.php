...
if (isset($_SESSION['orden_abm_t_maltrato']) && $_SESSION['orden_abm_t_maltrato'] != "") {
    $ordenarpor = $_SESSION['orden_abm_t_maltrato'];
    /* con_log("pasa"); */
} else {
    $ordenarpor = "aorden";
}
                ...
                <div class="card card-signin mb-5">
                    <div class="card-body">
                        <div class="row <? /* bg-custom-light titulo-seccion */ ?>">
                            <div class='col-6'><h5 class="abm-card-title mx-2">Tipo de Maltrato <span id="loader"></span></h5></div>
                            <div class="col-6" style='text-align: right;'>
                                <button onclick="limpiar_datos();" type='button' class="btn btn-custom" data-toggle="modal" data-target="#nuevoT_maltrato"><i class="fas fa-plus"></i> Agregar</button>
                            </div>
                        </div>
                        <?php
                        include("../modal/registro_t_maltrato.php");
                        include("../modal/editar_t_maltrato.php");
                        include("../modal/toggle_t_maltrato.php");
                        //include("../modal/cambiar_password.php");
                        ?>
                        <form class="form-horizontal" action="javascript:load(1, '<?= $ordenarpor ?>');" role="form" id="datos_t_maltrato">
                            <div class="form-group row justify-content-center">
                                <div class="col-8 col-md-6" mx-auto>
                                    <div class="input-group">

                                        <input type="text" class="form-control" id="q" placeholder="Buscar tipo de maltrato" <? /* onkeyup='load(1);' DESHABILITADO PARA EVITAR CONSUMO ALTO DE SERVER */ ?>>
...
<script>
                                    ...
                                    function load(page) {
                                        ...
                                        $("#loader").fadeIn('slow');
                                        $.ajax({
                                            url: '../ajax/buscar_t_maltrato.php?action=ajax&page=' + page + '&q=' + q + '&orden=' + orden,
                                    ...
                                    function mover(id, pos, dir, ordenar)
                                    {
                                        ...
                                        $.ajax({
                                            type: "GET",
                                            url: "../ajax/mover_t_maltrato.php?pos=" + pos + '&dir=' + dir,
                                            data: "id=" + id, "q": q,
                                    ...
                                    $("#toggle_form").submit(function (event) {
                                        ...
                                        $.ajax({
                                            type: "POST",
                                            url: "../ajax/toggle_t_maltrato.php",
                                            data: parametros,
                                            success: function (res) {
                                                if (res === "1") {
                                                    $("#alertabad").hide(500);   
                                                    $("#alertaok").hide(500);
                                                    $("#spanok").text('<?= $msg_t_mal[1] ?>');
                                                    $("#alertaok").show(500);
...
<script>
    $("#guardar_t_maltrato").submit(function (event) {
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res === "1") {
                        ...
                        $("#divnrok").hide(500);
                        $('#guardar_datos').attr("disabled", false);
                        $("#spannrerr").text('<?= $msg_nut_mal[3] ?>');
                        $("#divnrerr").show(500);
                    } else if (res === "4") {
                        $("#divnrok").hide(500);
                        $('#guardar_datos').attr("disabled", false);
                        $("#spannrerr").text('<?= $msg_nut_mal[4] ?>');
                        $("#divnrerr").show(500);
                    } else if (res === "") {
                        $("#divnrok").hide(500);
                        $('#guardar_datos').attr("disabled", false);
    ...
    function validaFormNV() {
        ...
        if (check_dato === "") {
            $("#divnrok").hide(500);
            $("#spannrerr").text("<?= $msg_nut_mal[5]["nombre1"] ?>");
            $("#divnrerr").show(500);
            $('#guardar_datos').attr("disabled", false);
            ...
            $("#divnrok").hide(500);
            $("#spannrerr").text("<?= $msg_nut_mal[5]["nombre2"] ?>");
            $("#divnrerr").show(500);
            $('#guardar_datos').attr("disabled", false);
            $("#descr").focus();       // Esta función coloca el foco de escritura del usuario en el campo Nombre directamente.
                    return false;
    ...
    $("#editar_t_maltrato").submit(function (event) {
        ...
        if (validaFormEV()) {
            ...
            $.ajax({
                type: "POST",
                url: "../ajax/editar_t_maltrato.php",
                data: parametros,
                success: function (res) {
                    if (res === "1") {
                        $("#divererr").hide(500);   
                        $('#actualizar_datos').attr("disabled", false);
                        $("#spanerok").text('<?= $msg_edt_mal[1] ?>');
                        $("#diverok").show(500);
                        load(1);
                    } else if (res === "3") {
                        $("#diverok").hide(500);
                        $('#actualizar_datos').attr("disabled", false);
                        $("#spanererr").text('<?= $msg_edt_mal[3] ?>');
                        $("#divererr").show(500);
                    } else if (res === "4") {
                        $("#diverok").hide(500);
                        $('#actualizar_datos').attr("disabled", false);
                        $("#spanererr").text('<?= $msg_edt_mal[4] ?>');
                        $("#divererr").show(500);
                    } else if (res === "") {
                        $("#diverok").hide(500);
                        $('#actualizar_datos').attr("disabled", false);
    ...
    function validaFormEV() {
        ...
        if (check_dato === "") {
            $("#diverok").hide(500);
            $("#spanererr").text("<?= $msg_nut_mal[5]["nombre1"] ?>");
            $("#divererr").show(500);
            $('#actualizar_datos').attr("disabled", false);
            $("#descr").focus();       // Esta función coloca el foco de escritura del usuario en el campo Nombre directamente.
                    return false;
            ...
            $("#diverok").hide(500);
            $("#spanererr").text("<?= $msg_nut_mal[5]["nombre2"] ?>");
            $("#divererr").show(500);
            $('#actualizar_datos').attr("disabled", false);
            $("#descr").focus();       // Esta función coloca el foco de escritura del usuario en el campo Nombre directamente.
                    return false;
    ...
    function obtener_datos_toggle(id, activo) {
        ...
        if (activo === 's') {
            ...
            $("#toggle_texto").text("<?= $msg_togt_mal['s']; ?>");
        } else {
            $("#toggle_titulo").text(" Deshabilitar ");
            $("#toggle_texto").text("<?= $msg_togt_mal['n']; ?>");
        }
        $("#alertabad").hide(500);
        $("#alertaok").hide(500);
    }
    $('#nuevoT_maltrato').on('shown.bs.modal', function () {
        $('#descr').focus();
    });

...
if (isset($_SESSION['orden_abm_m_problematica']) && $_SESSION['orden_abm_m_problematica'] != "") {
    $ordenarpor = $_SESSION['orden_abm_m_problematica'];
    /* con_log("pasa"); */
} else {
    $ordenarpor = "aorden";
}
                ...
                <div class="card card-signin mb-5">
                    <div class="card-body">
                        <div class="row <? /* bg-custom-light titulo-seccion */ ?>">
                            <div class='col-6'><h5 class="abm-card-title mx-2">Motivo de la Llamada (Problemáticas) <span id="loader"></span></h5></div>
                            <div class="col-6" style='text-align: right;'>
                                <button onclick="limpiar_datos();" type='button' class="btn btn-custom" data-toggle="modal" data-target="#nuevom_problematica"><i class="fas fa-plus"></i> Agregar</button>
                            </div>
                        </div>
                        <?php
                        include("../modal/registro_m_problematica.php");
                        include("../modal/editar_m_problematica.php");
                        include("../modal/toggle_m_problematica.php");
                        //include("../modal/cambiar_password.php");
                        ?>
                        <form class="form-horizontal" action="javascript:load(1, '<?= $ordenarpor ?>');" role="form" id="datos_m_problematica">
                            <div class="form-group row justify-content-center">
                                <div class="col-8 col-md-6" mx-auto>
                                    <div class="input-group">

                                        <input type="text" class="form-control" id="q" placeholder="Buscar motivo de la llamada (problemáticas)" <? /* onkeyup='load(1);' DESHABILITADO PARA EVITAR CONSUMO ALTO DE SERVER */ ?>>
...
<script>
                                    ...
                                    function load(page) {
                                        ...
                                        $("#loader").fadeIn('slow');
                                        $.ajax({
                                            url: '../ajax/buscar_m_problematica.php?action=ajax&page=' + page + '&q=' + q + '&orden=' + orden,
                                    ...
                                    function mover(id, pos, dir, ordenar)
                                    {
                                        ...
                                        $.ajax({
                                            type: "GET",
                                            url: "../ajax/mover_m_problematica.php?pos=" + pos + '&dir=' + dir,
                                            data: "id=" + id, "q": q,
                                    ...
                                    $("#toggle_form").submit(function (event) {
                                        ...
                                        $.ajax({
                                            type: "POST",
                                            url: "../ajax/toggle_m_problematica.php",
                                            data: parametros,
                                            success: function (res) {
                                                if (res === "1") {
                                                    $("#alertabad").hide(500);   
                                                    $("#alertaok").hide(500);
                                                    $("#spanok").text('<?= $msg_m_llam[1] ?>');
                                                    $("#alertaok").show(500);
...
<script>
    $("#guardar_m_problematica").submit(function (event) {
        ...
        if (validaFormNV()) {
            ...
            $.ajax({
                type: "POST",
                url: "../ajax/nuevo_m_problematica.php",
                data: parametros,
                success: function (res) {
                    if (res === "1") {
                        $("#divnrerr").hide(500);   
                        ...
                        $("#divnrok").hide(500);
                        $('#guardar_datos').attr("disabled", false);
                        $("#spannrerr").text('<?= $msg_num_llam[3] ?>');
                        $("#divnrerr").show(500);
                    } else if (res === "4") {
                        $("#divnrok").hide(500);
                        $('#guardar_datos').attr("disabled", false);
                        $("#spannrerr").text('<?= $msg_num_llam[4] ?>');
                        $("#divnrerr").show(500);
                    } else if (res === "") {
                        $("#divnrok").hide(500);
                        $('#guardar_datos').attr("disabled", false);
    ...
    function validaFormNV() {
        ...
        if (check_dato === "") {
            $("#divnrok").hide(500);
            $("#spannrerr").text("<?= $msg_num_llam[5]["nombre1"] ?>");
            $("#divnrerr").show(500);
            $('#guardar_datos').attr("disabled", false);
            $("#descr").focus();       // Esta función coloca el foco de escritura del usuario en el campo Nombre directamente.
                    return false;
        } else if (check_dato.trim().length < 3 || check_dato.trim().length > 45) {
            $("#divnrok").hide(500);
            $("#spannrerr").text("<?= $msg_num_llam[5]["nombre2"] ?>");
            $("#divnrerr").show(500);
            $('#guardar_datos').attr("disabled", false);
            $("#descr").focus();       // Esta función coloca el foco de escritura del usuario en el campo Nombre directamente.
                    return false;
    ...
    $("#editar_m_problematica").submit(function (event) {
        ...
        if (validaFormEV()) {
            ...
            $.ajax({
                type: "POST",
                url: "../ajax/editar_m_problematica.php",
                data: parametros,
                success: function (res) {
                    if (res === "1") {
                        $("#divererr").hide(500);   
                        $('#actualizar_datos').attr("disabled", false);
                        $("#spanerok").text('<?= $msg_edm_llam[1] ?>');
                        $("#diverok").show(500);
                        load(1);
                    } else if (res === "3") {
                        $("#diverok").hide(500);
                        $('#actualizar_datos').attr("disabled", false);
                        $("#spanererr").text('<?= $msg_edm_llam[3] ?>');
                        $("#divererr").show(500);
                    } else if (res === "4") {
                        $("#diverok").hide(500);
                        $('#actualizar_datos').attr("disabled", false);
                        $("#spanererr").text('<?= $msg_edm_llam[4] ?>');
                        $("#divererr").show(500);
                    } else if (res === "") {
                        $("#diverok").hide(500);
                        $('#actualizar_datos').attr("disabled", false);
    ...
    function validaFormEV() {
        ...
        if (check_dato === "") {
            $("#diverok").hide(500);
            $("#spanererr").text("<?= $msg_num_llam[5]["nombre1"] ?>");
            $("#divererr").show(500);
            $('#actualizar_datos').attr("disabled", false);
            $("#descr").focus();       // Esta función coloca el foco de escritura del usuario en el campo Nombre directamente.
                    return false;
            ...
            $("#diverok").hide(500);
            $("#spanererr").text("<?= $msg_num_llam[5]["nombre2"] ?>");
            $("#divererr").show(500);
            $('#actualizar_datos').attr("disabled", false);
            $("#descr").focus();       // Esta función coloca el foco de escritura del usuario en el campo Nombre directamente.
                    return false;
    ...
    function obtener_datos_toggle(id, activo) {
        ...
        if (activo === 's') {
            ...
            $("#toggle_texto").text("<?= $msg_togm_llam['s']; ?>");
        } else {
            $("#toggle_titulo").text(" Deshabilitar ");
            $("#toggle_texto").text("<?= $msg_togm_llam['n']; ?>");
        }
        $("#alertabad").hide(500);
        $("#alertaok").hide(500);
    }
    $('#nuevom_problematica').on('shown.bs.modal', function () {
        $('#descr').focus();
    });

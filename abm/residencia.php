...
if (isset($_SESSION['orden_abm_residencia']) && $_SESSION['orden_abm_residencia'] != "") {
    $ordenarpor = $_SESSION['orden_abm_residencia'];
    /* con_log("pasa"); */
} else {
    $ordenarpor = "aorden";
}
                ...
                <div class="card card-signin mb-5">
                    <div class="card-body">
                        <div class="row <? /* bg-custom-light titulo-seccion */ ?>">
                            <div class='col-6'><h5 class="abm-card-title mx-2">Lugar de Residencia <span id="loader"></span></h5></div>
                            <div class="col-6" style='text-align: right;'>
                                <button onclick="limpiar_datos();" type='button' class="btn btn-custom" data-toggle="modal" data-target="#nuevoResidencia"><i class="fas fa-plus"></i> Agregar</button>
                            </div>
                        </div>
                        <?php
                        include("../modal/registro_residencia.php");
                        include("../modal/editar_residencia.php");
                        include("../modal/toggle_residencia.php");
                        //include("../modal/cambiar_password.php");
                        ?>
                        <form class="form-horizontal" action="javascript:load(1, '<?= $ordenarpor ?>');" role="form" id="datos_residencia">
                            <div class="form-group row justify-content-center">
                                <div class="col-8 col-md-6" mx-auto>
                                    <div class="input-group">

                                        <input type="text" class="form-control" id="q" placeholder="Buscar lugar de residencia" <? /* onkeyup='load(1);' DESHABILITADO PARA EVITAR CONSUMO ALTO DE SERVER */ ?>>
...
<script>
                                    ...
                                    function load(page) {
                                        ...
                                        $("#loader").fadeIn('slow');
                                        $.ajax({
                                            url: '../ajax/buscar_residencia.php?action=ajax&page=' + page + '&q=' + q + '&orden=' + orden,
                                    ...
                                    function mover(id, pos, dir, ordenar)
                                    {
                                        ...
                                        $.ajax({
                                            type: "GET",
                                            url: "../ajax/mover_residencia.php?pos=" + pos + '&dir=' + dir,
                                            data: "id=" + id, "q": q,
                                    ...
                                    $("#toggle_form").submit(function (event) {
                                        ...
                                        $.ajax({
                                            type: "POST",
                                            url: "../ajax/toggle_residencia.php",
                                            data: parametros,
                                            success: function (res) {
                                                if (res === "1") {
                                                    $("#alertabad").hide(500);   
...
<script>
    $("#guardar_residencia").submit(function (event) {
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res === "1") {
                        ...
                        $("#divnrok").hide(500);
                        $('#guardar_datos').attr("disabled", false);
                        $("#spannrerr").text('<?= $msg_nuresid[3] ?>');
                        $("#divnrerr").show(500);
                    } else if (res === "4") {
                        $("#divnrok").hide(500);
                        $('#guardar_datos').attr("disabled", false);
                        $("#spannrerr").text('<?= $msg_nuresid[4] ?>');
                        $("#divnrerr").show(500);
                    } else if (res === "") {
                        $("#divnrok").hide(500);
                        $('#guardar_datos').attr("disabled", false);
    ...
    function validaFormNR() {
        // Campos de texto
        check_dato = $("#descr").val();

        if (check_dato === "") {
            $("#divnrok").hide(500);
            $("#spannrerr").text("<?= $msg_nuresid[5]["nombre1"] ?>");
            $("#divnrerr").show(500);
            $('#guardar_datos').attr("disabled", false);
            $("#descr").focus();       // Esta función coloca el foco de escritura del usuario en el campo Nombre directamente.
                    return false;
        } else if (check_dato.trim().length < 3 || check_dato.trim().length > 100) {
            $("#divnrok").hide(500);
            $("#spannrerr").text("<?= $msg_nuresid[5]["nombre2"] ?>");
            $("#divnrerr").show(500);
            $('#guardar_datos').attr("disabled", false);
            $("#descr").focus();       // Esta función coloca el foco de escritura del usuario en el campo Nombre directamente.
                    return false;
    ...
    $("#editar_residencia").submit(function (event) {
        ...
        if (validaFormER()) {
            ...
            $.ajax({
                type: "POST",
                url: "../ajax/editar_residencia.php",
                data: parametros,
                success: function (res) {
                    if (res === "1") {
                        $("#divererr").hide(500);   
                        $('#actualizar_datos').attr("disabled", false);
                        $("#spanerok").text('<?= $msg_edresid[1] ?>');
                        $("#diverok").show(500);
                        load(1);
                    } else if (res === "3") {
                        $("#diverok").hide(500);
                        $('#actualizar_datos').attr("disabled", false);
                        $("#spanererr").text('<?= $msg_edresid[3] ?>');
                        $("#divererr").show(500);
                    } else if (res === "4") {
                        $("#diverok").hide(500);
                        $('#actualizar_datos').attr("disabled", false);
                        $("#spanererr").text('<?= $msg_edresid[4] ?>');
                        $("#divererr").show(500);
                    } else if (res === "") {
                        $("#diverok").hide(500);
                        $('#actualizar_datos').attr("disabled", false);
    ...
    function validaFormER() {
        // Campos de texto
        check_dato = $("#descr2").val();

        if (check_dato === "") {
            $("#diverok").hide(500);
            $("#spanererr").text("<?= $msg_nuresid[5]["nombre1"] ?>");
            $("#divererr").show(500);
            $('#actualizar_datos').attr("disabled", false);
            $("#descr").focus();       // Esta función coloca el foco de escritura del usuario en el campo Nombre directamente.
                    return false;
            ...
            $("#diverok").hide(500);
            $("#spanererr").text("<?= $msg_nuresid[5]["nombre2"] ?>");
            $("#divererr").show(500);
            $('#actualizar_datos').attr("disabled", false);
            $("#descr").focus();       // Esta función coloca el foco de escritura del usuario en el campo Nombre directamente.
                    return false;
    ...
    function obtener_datos_toggle(id, activo) {
        ...
        if (activo === 's') {
            $("#toggle_resid_titulo").text(" Habilitar ");
            $("#toggle_resid_texto").text("<?= $msg_togresid['s']; ?>");
        } else {
            $("#toggle_resid_titulo").text(" Deshabilitar ");
            $("#toggle_resid_texto").text("<?= $msg_togresid['n']; ?>");
        }
        $("#alertabad").hide(500);
        $("#alertaok").hide(500);
    }
    $('#nuevoResidencia').on('shown.bs.modal', function () {
        $('#descr').focus();
    });
